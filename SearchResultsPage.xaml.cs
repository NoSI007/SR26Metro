...
namespace W8rtmGrid
{
    ...
    public sealed partial class SearchResultsPage : Page
    {
        ...
        private void navigationHelper_LoadState(object sender, LoadStateEventArgs e)
        {
            ...
            //------------------------------------------------------
            //2.TODO the code goes here
            var groups = SampleDataSource.Allgroups();
            string qry = queryText.ToLower();
            ...
            foreach (var grp in groups)
            {
                ...
                {
                    if (item.Description.ToLower().Contains(qry) == true)
                    {
                        all.Add(item);
                        items.Add(item);
                    }
                ...
                if (items.Count > 0)
                {
                    _results.Add(grp.Title, items);

                    filterList.Add(new Filter(grp.Title, items.Count, false));
                }
        ...
        void Filter_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (selectedFilter != null)
            {
                ...
                this.defaultViewModel["Results"] = _results[selectedFilter.Name];// required response.

                if (selectedFilter.Name.Equals("All"))
                {
                    AllResults();
                }
                else if (_results.ContainsKey(selectedFilter.Name))
                {
                    this.DefaultViewModel["Results"] = new List<SampleDataItem>(_results[selectedFilter.Name]);
                }
        ...
        private void AllResults()
        {
            ...
            foreach (var group in _results)
            {
                ...
                break;// only the the first list is needed for all the results.
            }
        ...
        private void OnClick(object sender, ItemClickEventArgs e)
        {
            ...
            string iid = ((SampleDataItem)e.ClickedItem).UniqueId;


            this.Frame.Navigate(typeof(Detail4Food), iid);
        }
